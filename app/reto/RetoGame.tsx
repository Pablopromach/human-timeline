...
import { ArrowLeft, Search, Trophy, Share2, Sparkles, RotateCcw, X } from 'lucide-react'
...
export default function RetoGame() {
  ...
  const results = useMemo(
    () => searchFigures(query, allFigures, 6),
    [query]
  )
  ...
  const shareResults = useCallback(async () => {
    ...
    const text = `🏛️ Human Timeline · Reto Histórico

${rating.emoji} ${rating.label}
Puntuación: ${score} / ${TOTAL_ROUNDS * 10}

${history.map((h, i) => {
  const ico = h.result.status === 'perfect' ? '🟢' :
              h.result.status === 'close' ? '🟡' :
              h.result.status === 'fair' ? '⚪' :
              h.result.status === 'far' ? '🟠' : '🔴'
  return `${ico} Ronda ${i + 1}: ${h.year < 0 ? Math.abs(h.year) + ' aC' : h.year}`
}).join('\n')}

Juega tú: https://human-timeline.vercel.app/reto`

    if (navigator.share) {
      try {
        await navigator.share({ title: 'Mi puntuación en Human Timeline', text })
      } catch {}
        ...
        <div className="flex-1 flex items-center justify-center px-6 py-12">
          ...
          >
            ...
            <p className="text-white/55 text-base mb-10 leading-relaxed">
              Te damos un año. Tú buscas un personaje histórico que viviera entonces.
              Cuanto más cerca esté, más puntos sumas. 10 rondas. Puntuación final compartible.
            </p>
          ...
          >
            <div className="text-center mb-8">
              ...
              <div className="text-7xl font-bold font-mono" style={{ color: 'rgba(255,255,255,0.92)' }}>
                {score}
                <span className="text-white/25 text-3xl">/{maxScore}</span>
              </div>
            </div>
        ...
        <div className="max-w-3xl mx-auto">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-sm font-mono">
              <span className="text-white/30 mr-2">Puntuación</span>
              <span
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="text-[11px] text-white/35 font-mono mt-0.5">
                                  {formatYear(r.figure.birthYear)} – {formatYear(r.figure.deathYear)} · {r.figure.country}
                                </div>
                              </div>
                              <span
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Mini timeline showing how close to target */}
        <YearDistanceBar
...
function YearDistanceBar({ targetYear, birthYear, deathYear, color }: {
  ...
  return (
    <div className="relative h-12">
      ...
      {/* Year markers */}
      <div
        className="absolute -bottom-1 text-[9px] font-mono text-white/40 transform -translate-x-1/2"
        style={{ left: `${lifeStart}%` }}
      >
        {birthYear < 0 ? Math.abs(birthYear) + 'aC' : birthYear}
      </div>
      <div
        className="absolute -bottom-1 text-[9px] font-mono text-white/40 transform -translate-x-1/2"
        style={{ left: `${lifeStart + lifeWidth}%` }}
      >
